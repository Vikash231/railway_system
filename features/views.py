...
def getTrains(request):
    if request.method=='POST':
        ...
        src_station = Station.objects.get(sid=src)
        des_station = Station.objects.get(sid=des)

        # src_lat = src_station.latitude
        # src_long = src_station.longitude
        # des_lat = des_station.latitude
        # des_long = des_station.longitude


        distance = calculate_distance(src_station , des_station)



        return render(request,'features/trains.html',{'data':x,'o':o,'src':src,'des':des , 'distance': distance})
